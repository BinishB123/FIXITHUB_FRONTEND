...
export function ChatComponenet() {
  ...
  useEffect(() => {
    getChatOfOneToOne(params.chatid + "").then((response: any) => {
      ...
      socket?.emit("oppositeGuysIsInOnlineOrNot", { userId: params.providerid, emitTo: userInfo?.id })
    })
  ...
  const sendMessage = (sender: string, chatId: string, message: string, recieverId: string) => {
    ...
    <div className="w-[100%] md:w-[80%] h-[600px]  flex space-x-1">
      ...
      <div className=" hidden w-[40%] h-[600px] animate-fadeInDownBig bg-banner-gray rounded-md flex flex-col items-center s ">
        <div className="w-[90%] h-[80px]   border-b-2 place-items-center">
          <h1 className="text-md font-dm font-medium text-center text-white mt-4">Chat With Provider</h1>
          <div className="h-[32px] w-[100%] flex justify-between" >
            <input type="text" className="h-[32px] w-[90%] rounded-sm text-white text-sm font-sm font-semibold bg-banner-gray outline-none" placeholder="Search with Provider Name.." name="" id="" />
      ...
      <div className="w-[100%] md:w-[60%] h-[600px]  flex flex-col animate-fadeInDownBig">
        ...
        > <div className="w-[10%] ml-2 h-[55px] mt-3  rounded-full  overflow-hidden flex items-center justify-center">
            <img src={chatConverstaion?.provider.logoUrl} alt="" className="w-[90%] h-[90%] object-cover" />
          ...
          <div className="w-[60%] flex flex-col">
            <div className="w-[100%] h-[30px] ">
              <h1 className="truncate text-white mt-3">{chatConverstaion?.provider.workshopName}</h1>
            </div>
            <div className="w-[100%] h-[80px]">
              {
                online === true ? <h1 className="truncate text-green-500 text-sm mt-3 animate-pulse  ">{(online && !typing.indicate) ? "online" : (typing.indicate && typing.typer === "provider") && typing.indicate}</h1>
        ...
        <div className="w-[100%] h-[570px] ">
          <div className="w-[100%] flex justify-center  bg-gradient-to-b from-gray-950 h-[470px] ">
            <div ref={messagedivref} className="w-[90%] h-[460px] overflow-y-scroll scrollbar-hide mt-2 space-y-3">
              {chatConverstaion?.messages && chatConverstaion?.messages.map((data, index) => (
                <>
                  {
                    data.sender !== "user" ? <div key={index} className="w-[100%] flex justify-start">
                      <div className="w-[60%] h-[]  bg-banner-gray rounded-md ">
                        <p className=" text-white break-words text-sm text-start ml-2 mt-1 mb-2 font-medium rounded-md">{data.message}</p>
                        <p className="text-end text-sm text-white mr-3  tracking-tighter">{`${v.getHours()} : ${v.getMinutes()}`}</p>

                      </div>
                    </div> : <div className="w-[100%] flex justify-end">
          ...
          <div className="w-[100%] h-[50px]   flex space-x-6 ">
            <input value={message} onChange={(e) => {
              ...
              className="w-[100%] text-white pl-3 bg-banner-gray h-[40px] rounded-md " placeholder="Type a message" type="text" name="" id="" />
            <IoSend className="text-orange text-4xl" onClick={() => {
              if (chatConverstaion) {
                sendMessage("user", chatConverstaion?._id + "", message, chatConverstaion?.provider._id)
              }
            }} />
          </div>
