...
function ProviderInCommigCallModal(props:{success:boolean|null, getter:string|null, chatid: string|null,setModal:()=>void, callerData:{name:string|null,logoUrl:string|null}|any}){ 
  const {socket} = useSocket()
  const navigate = useNavigate()

    const onClickAccept = ()=>{
      socket?.emit("Accepted",props)
      navigate(`/provider/call/${props.getter}`,{state:{incomingcall:true}})
      props.setModal()
      ...
      >
        ...
        <div className=" w-[80px] h-[100px] md:w-[130px] md:h-[150px] mt-6  flex justify-center items-center rounded-full overflow-hidden">
            ...
            <div className="w-[100%] flex justify-center space-x-5 cursor-pointer">
             <div className="w-[12%] h-[40px]  md:h-[55px] rounded-full animate-fadeInDown bg-red flex justify-center items-center  " onClick={onClickReject}>
              <IoCloseSharp  className="text-white text-5xl"  />
              </div>
              <div className="w-[12%] h-[40px]  md:h-[55px] rounded-full  bg-green-500 flex justify-center items-center animate-bounce  " onClick={onClickAccept}>
...
export default ProviderInCommigCallModal